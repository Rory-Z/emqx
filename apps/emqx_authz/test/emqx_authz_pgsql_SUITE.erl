...
t_authz(_) ->
    ...
    ?assertEqual(deny, emqx_authz:check(ClientInfo1, subscribe, <<"#">>)), % nomatch
    ?assertEqual(deny, emqx_authz:check(ClientInfo1, publish, <<"#">>)), % nomatch

    meck:expect(emqx_resource, query, fun(_, _) -> {ok, ?COLUMNS, ?RULE1 ++ ?RULE2} end),
    ?assertEqual(deny, emqx_authz:check(ClientInfo1, subscribe, <<"+">>)),
    ?assertEqual(deny, emqx_authz:check(ClientInfo1, publish, <<"+">>)),

    meck:expect(emqx_resource, query, fun(_, _) -> {ok, ?COLUMNS, ?RULE2 ++ ?RULE1} end),
    ?assertEqual(allow, emqx_authz:check(ClientInfo1, subscribe, <<"#">>)),
    ?assertEqual(deny, emqx_authz:check(ClientInfo2, subscribe, <<"+">>)),

    meck:expect(emqx_resource, query, fun(_, _) -> {ok, ?COLUMNS, ?RULE3 ++ ?RULE4} end),
    ?assertEqual(allow, emqx_authz:check(ClientInfo2, subscribe, <<"test/test_clientid">>)),
    ?assertEqual(deny,  emqx_authz:check(ClientInfo2, publish,   <<"test/test_clientid">>)),
    ?assertEqual(deny,  emqx_authz:check(ClientInfo2, subscribe, <<"test/test_username">>)),
    ?assertEqual(allow, emqx_authz:check(ClientInfo2, publish,   <<"test/test_username">>)),
    ?assertEqual(deny,  emqx_authz:check(ClientInfo3, subscribe, <<"test">>)), % nomatch
    ?assertEqual(deny,  emqx_authz:check(ClientInfo3, publish,   <<"test">>)), % nomatch
    ok.
